...
class DownloadSignalBus(QObject):
    """
    Singleton Signal Bus for bridging background download threads to the GUI.
    Ensures thread-safe communication for CMSIS-Pack download events.
    """
    _instance = None
    ...
    @classmethod
    def instance(cls):
        """Returns the singleton instance of the DownloadSignalBus."""
        if cls._instance is None:
            cls._instance = DownloadSignalBus()
        return cls._instance
...
class GlobalDownloadDialog(QDialog):
    """
    A persistent, modal dialog that freezes the main application while a 
    hardware CMSIS-Pack is being downloaded. Displays live progress.
    """
    ...
    def __init__(self, parent=None):
        ...
        # Disable the standard window close button (X) to force controlled cancellation
        self.setWindowFlags(
        ...
        # Apply unified industrial dark theme
        self.setStyleSheet("""
            QDialog {
                background-color: #070B19;
                border: 2px solid #00E5FF;
        ...
        # Connect global signals to this UI instance
        bus = DownloadSignalBus.instance()
        bus.dialog_instance = self
        bus.download_started.connect(self._on_download_started)
        bus.download_finished.connect(self._on_download_finished)
    ...
    def ask_permission(self, target_name: str) -> bool:
        """
        Interrupts the user to ask for download permission.
        Must be executed in the main GUI thread.
        """
        # Ensure the cursor is restored from a 'WaitCursor' state before showing the dialog
        while QApplication.overrideCursor() is not None:
            QApplication.restoreOverrideCursor()
    ...
    def _on_cancel_clicked(self):
        """Handles the cancellation request from the user."""
        self.lbl_info.setText("Cancelling... Stopping processes.")
        self.progress_bar.setRange(0, 0)
        self.btn_cancel.setEnabled(False)
        DownloadSignalBus.instance().cancel_requested = True
    ...
    @Slot(str)
    def _on_download_started(self, target_name: str):
        """Triggered when the background download formally begins."""
        self.btn_cancel.setEnabled(True)
        DownloadSignalBus.instance().cancel_requested = False
        self.lbl_info.setText(
            f"Missing definitions for {target_name}. Fetching from ARM...")
    ...
    @Slot(int, str)
    def _on_download_progress(self, pct: int, msg: str):
        """Updates the progress bar and status label in real-time."""
        if pct == 100:
            self.progress_bar.setRange(0, 0)
            self.lbl_info.setText(
                "Download complete!\nFinalizing installation & configuring core cache. Almost done...")
            ...
            # Indeterminate progress (e.g., during extraction or initialization)
            self.progress_bar.setRange(0, 0)
            if msg:
                self.lbl_info.setText(msg)
    ...
    @Slot(bool, str)
    def _on_download_finished(self, success: bool, message: str):
        """Cleans up the UI once the download succeeds or fails."""
        while QApplication.overrideCursor() is not None:
            QApplication.restoreOverrideCursor()
        self.hide()
