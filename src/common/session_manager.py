...
# and unified memory read/write operations. (Enhanced with verbose terminal debugging)
# """
...
#         try:
#             print("[DEBUG-SESSION USB_PROBE] Running command: dfu-util -l")
...
#             """Close a pyOCD session and swallow any errors, so the USB/HID
#             handle is always released before we try to open another one."""
#             if sess:
#                 try:
#                     print(
...
#             """Return the real ARM core architecture (e.g. 'CORTEX-M3'),
#             read from the target's core objects — NOT session.board.target_type,
#             which is just the target-selection string (e.g. 'stm32f103rb').

#             core.core_type is an integer code on this pyOCD version (not an
#             enum with .name), so it must be mapped through pyOCD's own
#             CORE_TYPE_NAME lookup table.
#             """
#             try:
#                 core = tgt.cores[0]
#                 code = core.core_type
...
#                 except ImportError:
#                     # Fallback if the table moved/renamed in this pyOCD version.
...
#                 blocking=False, options=options)

#             if session is None:
#                 raise Exception("No DAPLink/SWD probe detected on USB.")
...
#             # Always release the USB/HID handle from the failed attempt
#             # BEFORE trying to open a new session on the same physical probe.
#             _safe_close(session)
#             session = None
...
#                 print(
#                     "[DEBUG-SESSION PROBE_INFO] Target not recognized. Attempting fallback to 'cortex_m'...")
#                 try:
...
#                 info["error"] = str(e)
#         finally:
#             # Whatever session is currently alive (primary success, fallback
#             # success, or fallback failure) gets closed here exactly once.
#             _safe_close(session)
...
#             return True
#         except Exception as e:
#             print(f"[DEBUG-SESSION _open_session EXCEPTION] Failed: {str(e)}")
...
class SessionManager:
    ...
    def __init__(
        ...
        # ⬅️ اضافه شدن پارامتر اختیاری برای پشتیبانی از چند پروب
        ...
        self.connect_mode = connect_mode
        self.interface_type = interface_type
        # ⬅️ ذخیره شناسه یکتای پروب (در صورت ارسال)
    ...
    def probe_target_info(self, clock_freq: int = 1000000) -> Dict[str, Any]:
        ...
        try:
            ...
            session = ConnectHelper.session_with_chosen_probe(
                blocking=False,
                options=options,
                unique_id=self.unique_id  # ⬅️ ارسال آیدی پروب برای پیدا کردن دستگاه هدف در صورت وجود
            )
            ...
            if "not recognized" in err_lower or "target" in err_lower:
                ...
                try:
                    ...
                    session = ConnectHelper.session_with_chosen_probe(
                        blocking=False,
                        options=options,
                        # ⬅️ استفاده از آیدی در حالت بازگشتی (Fallback)
    ...
    def _open_session(
        ...
        try:
            ...
            # ⬅️ ارسال شناسه پروب به ConnectHelper
            self.session = ConnectHelper.session_with_chosen_probe(
                options=options,
                unique_id=self.unique_id  # اگر None باشد به اولین دستگاه وصل می‌شود
            )
