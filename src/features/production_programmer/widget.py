...
from PySide6.QtWidgets import (
    ...
    QFrame,
    QSizePolicy,
    QApplication,  # ⬅️ اضافه شدن QApplication برای تغییر شکل ماوس و قفل صفحه
)
from PySide6.QtGui import QPainter, QPolygon, QIcon
...
class ProductionProgrammerWidget(QWidget):
    ...
    def _start_production_flash(self) -> None:
        ...
        # ⬅️ اتصال سیگنال‌های دانلود درایور به رابط کاربری
        self._worker.pack_download_started_signal.connect(
            self._on_pack_download_started)
        self._worker.pack_download_finished_signal.connect(
            self._on_pack_download_finished)
    ...
    # ==========================================================
    # ⬅️ متدهای جدید برای قفل کردن و آزاد کردن رابط کاربری
    # ==========================================================
    @Slot(str)
    def _on_pack_download_started(self, mcu_name: str) -> None:
        """قفل کردن کل برنامه و نمایش پیام دانلود پکیج"""
        QApplication.setOverrideCursor(Qt.CursorShape.WaitCursor)
        self.qa_banner.set_busy_state(
            f"DOWNLOADING HARDWARE DRIVER...\nFetching definitions for {mcu_name} from ARM Global Index."
        )

    @Slot(bool, str)
    def _on_pack_download_finished(self, success: bool, message: str) -> None:
        """آزاد کردن موس پس از اتمام دانلود"""
        QApplication.restoreOverrideCursor()
        if success:
            self.qa_banner.set_busy_state(
                "Driver download complete. Resuming operation...")
            logger.info("Driver downloaded and UI unlocked.")
        else:
            QMessageBox.critical(
                self, "Download Failed", f"Could not fetch hardware driver from ARM Index:\n\n{message}")
            self.qa_banner.set_fail_state("Driver Download Failed.", 0.0)
