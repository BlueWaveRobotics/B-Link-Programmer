...
class ProductionProgrammerWorker(BaseWorker):
    ...
    def __init__(
        ...
        serial_address: int = 0x0801FC00,
        interface_type: str = "DAPLink (SWD)",
        target_type: Optional[str] = None,  # ⬅️ اضافه شدن نام میکرو به ورودی
        parent: Optional[Any] = None,
    ):
        super().__init__(parent)
        self.file_path = file_path
        ...
        self.serial_address = serial_address
        self.interface_type = interface_type
        self.target_type = target_type  # ⬅️ ذخیره نام میکرو

        print(
            f"[DEBUG-PROD-WORKER] Initialized. Target: {self.target_type}, Interface: {self.interface_type}, Clock: {self.clock_freq}Hz, Mode: {self.connect_mode}")
    ...
    def run_chip_erase(self) -> None:
        ...
        # ⬅️ ارسال نام میکرو به SessionManager
        sm = SessionManager(
        ...
        try:
            ...
            if "USB" in self.interface_type:
                ...
                print(
                    "[DEBUG-PROD-WORKER Erase Step 3.5-SWD] Forcing Halt to prevent Flash Algorithm timeout...")
                sm.reset_target(halt=True)
                time.sleep(0.1)
    ...
    def run_production_flash(self) -> None:
        ...
        try:
            ...
            # ⬅️ حذف کدهای قدیمی UID و جایگزینی با رویکرد یونیورسال
            print(
                "[DEBUG-PROD-WORKER Flash Step 3] UID reading skipped (Universal Mode).")
            self.log(
                "[INFO] Target UID reading bypassed to maintain universal compatibility.")
