...
#                 cmd = [
#                     EXE_DFU,  # ⬅️ اصلاح شد: استفاده از متغیر مسیر یاب بدون کوتیشن
#                     "-a", "0",
...
#                 print(
#                     f"[DEBUG-PROD-WORKER Erase Step 4-DFU] Executing DFU command: {' '.join(cmd)}")
...
#                 # سیستم تلاش مجدد و ریکاوری خودکار (Auto-Recovery)
#                 if not sm.erase_chip():
#                     print(
#                         "[DEBUG-PROD-WORKER Erase Step 4-SWD] First attempt failed. Attempting Auto-Recovery...")
#                     self.log(
#                         "[WARNING] Erase interrupted by running code. Running auto-recovery...")

#                     # الف) میکرو را به اجبار متوقف می‌کنیم
#                     try:
...
#                     # ج) دوباره دستور پاکسازی می‌دهیم
#                     if not sm.erase_chip():
#                         print(
...
#                 # ⬅️ اصلاح شد: استفاده از متغیر EXE_DFU بدون کوتیشن
#                 cmd = [EXE_DFU, "-a", "0"]
...
#                 print(
#                     f"[DEBUG-PROD-WORKER Flash Step 5-DFU] Executing DFU command: {' '.join(cmd)}")
...
class ProductionProgrammerWorker(BaseWorker):
    ...
    # ⬅️ اضافه شدن سیگنال‌های مربوط به دانلود درایور (Pack)
    pack_download_started_signal = Signal(str)
    pack_download_finished_signal = Signal(bool, str)
    ...
    def run_chip_erase(self) -> None:
        ...
        # ⬅️ پاس دادن سیگنال‌ها به عنوان کالبک به SessionManager
        sm = SessionManager(
            connect_mode=self.connect_mode,
            clock_freq=self.clock_freq,
            interface_type=self.interface_type,
            download_start_callback=self.pack_download_started_signal.emit,
            download_finish_callback=self.pack_download_finished_signal.emit
        )
