...
# /* استایل دکمه‌های کنترل عملیات - استفاده از رنگ‌های موفقیت و خطا برای هاور */
...
# /* منوی کشویی - استفاده از آیکون SVG سفید */
# QComboBox {
#     background-color: #070B19;
#     border: 1px solid #1A2642;
#     border-radius: 4px;
...
# /* کارت اسلات‌ها - اصلاح رنگ‌بندی کادر و پس‌زمینه ثابت */
# QGroupBox#ProbeSlotCard {
#     border: 1px solid #1A2642; /* کادر پیش‌فرض */
#     border-radius: 4px;
#     background-color: #0C1327; /* پس‌زمینه ثابت */
#     margin-top: 4px;
#     padding-top: 10px;
# }
# QGroupBox#ProbeSlotCard[status="PASS"] {
#     border-left: 4px solid #10B981; /* کادر سبز زمردی موفقیت */
# }
# QGroupBox#ProbeSlotCard[status="FAIL"] {
#     border-left: 4px solid #EF4444; /* کادر قرمز خطا */
# }
# QGroupBox#ProbeSlotCard[status="BUSY"] {
#     border-left: 4px solid #00E5FF; /* کادر سایان نئونی پردازش */
# }
# QGroupBox#ProbeSlotCard[status="NORMAL"] {
#     border-left: 4px solid #1A2642; /* کادر پیش‌فرض برای حالت عادی */
# }

# /* ترمینال (Log) - پس‌زمینه مشکی و متن سبز زمردی */
...
#         # اعمال تم و استایل‌های اصلی BlueWave
...
#         # استرچ صفر تا کمترین فضای ممکن را اشغال کند
...
#         # اصلاح کلیدی: حذف AlignTop تا اسلات‌ها به بالای کادر نچسبند و فضا باز شود *
#         self.scroll_area.setWidget(self.slots_container)

#         slots_main_layout.addWidget(self.scroll_area, stretch=1)

#         # اصلاح کلیدی ۱: دادن استرچ منصفانه (stretch=2) تا هم پنل فشرده شود و هم اسکرول ایجاد نشود *
...
#         # اصلاح کلیدی ۳: استفاده از آیکون‌های استاندارد مونوکروم سفید *
#         self.btn_start_batch.setIcon(
#             # آیکون play جدید
#             QIcon("assets/icons/pen-to-square-solid-full.svg"))
#         self.btn_start_batch.setMinimumHeight(
#             40)  # کاهش ارتفاع دکمه برای ذخیره فضا
...
#             QIcon("assets/icons/eraser-solid-full.svg"))  # آیکون trash جدید
...
#             QIcon("assets/icons/stop-solid-full.svg"))  # آیکون stop جدید
...
#         # حداقل ارتفاع مناسب داده شد تا خیلی جمع نشود
#         self.log_viewer.setMinimumHeight(120)

#         # اصلاح کلیدی: دادن وزن متعادل (stretch=2) به ترمینال *
...
#             # 🔵 تنظیم ऑब्जेक्ट نیم برای اعمال استایل‌های کارت در QSS
#             card.setObjectName("ProbeSlotCard")
#             # 🔵 تنظیم پراپرتی داینامیک پیش‌فرض (این تغییر باید در فایل probe_card اعمال شود)
...
#                 # 🔵 اصلاح کلیدی ۲: تغییر پراپرتی وضعیت برای نمایش کادر سایان نئونی در کارت *
...
#         # 🔵 اصلاح کلیدی ۳: استفاده از آیکون‌های مونوکروم سفید برای حالت لودینگ *
#         # آیکون hourglass جدید که تک‌رنگ سفید است
#         self.btn_start_batch.setIcon(
...
#             # 🔵 تنظیم پراپرتی وضعیت برای نمایش کادر سایان نئونی
#             card.setProperty("status", "BUSY")
#         elif status_code == "PASS":
#             card.set_pass_state(cycle_time=cycle_time, uid_str=chip_uid)
#             # 🔵 تنظیم پراپرتی وضعیت برای نمایش کادر سبز زمردی موفقیت
...
#             # 🔵 تنظیم پراپرتی وضعیت برای نمایش کادر قرمز خطا
#             card.setProperty("status", "FAIL")
#             self._append_log(f"[FAIL] Slot [{unique_id[:8]}]: {message}")

#         # 🔵 اعمال مجدد استایل برای رندر شدن کادر جدید
#         card.style().unpolish(card)
#         card.style().polish(card)
...
#         # بازگرداندن وضعیت دکمه‌ها و آیکون‌ها به حالت عادی با آیکون‌های وکتور سفید جدید
#         self.btn_start_batch.setIcon(
#             QIcon("assets/icons/pen-to-square-solid-full.svg"))
...
/* 🔵 1. دکمه Start Batch: متن و کادر آبی در حالت عادی، پس‌زمینه پررنگ در هاور */
...
/* 🔴 2. دکمه Full Chip Erase: متن و کادر قرمز در حالت عادی، پس‌زمینه پررنگ در هاور */
...
/* ⚪ 3. دکمه Stop All: هم‌رنگ و هم‌استایل دکمه‌های عادی برنامه */
QPushButton#btnStop {
    background-color: #121D38;
    color: #F8FAFC;
    border: 1px solid #1A2642;
...
class BatchProgrammerWidget(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        self.btn_stop.setObjectName("btnStop")
        self.btn_stop.setIcon(
            QIcon("assets/icons/stop-solid-full.svg"))
        self.btn_stop.setMinimumHeight(40)
        self.btn_stop.setFixedWidth(110)
        self.btn_stop.clicked.connect(self._stop_batch_execution)
        self.btn_stop.setEnabled(False)
