...
from src.common.resources import ICON_ARROWS_ROTATE, ICON_BOLT, ICON_HOURGLASS
from typing import Optional, Dict, Any
from PySide6.QtCore import Qt, Slot, QSize
from PySide6.QtGui import QFont, QIcon
from PySide6.QtWidgets import (
...
class OptionBytesWidget(QWidget):
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self.current_mcu_target: str = "auto"  # 🌟 متغیر ذخیره نام میکرو
    ...
    def set_mcu_target(self, target_name: str) -> None:
        """ 🌟 دریافت نام میکرو از تب اصلی """
        self.current_mcu_target = target_name

        # اگر میکرو غیر از ST باشد، یک هشدار ملایم به کاربر می‌دهیم
        if "lpc" in target_name.lower() or "nrf" in target_name.lower() or "rp2040" in target_name.lower():
            self.lbl_rdp_status.setText(
                f"Info: Target '{target_name.upper()}' uses different security architecture.")
            self.lbl_rdp_status.setStyleSheet(
                "color: #F59E0B; font-size: 12px; font-weight: bold;")
        else:
            self.lbl_rdp_status.setText("Current Status: Pending Read...")
            self.lbl_rdp_status.setStyleSheet(
                "color: #94A3B8; font-size: 12px; font-weight: bold;")

    def set_interface_type(self, interface_type: str) -> None:
        pass  # Reserved for future USB DFU Option Bytes handling

    def _apply_styles(self) -> None:
        # کدهای استایل شما دست‌نخورده باقی می‌ماند (حذف شد تا پاسخ طولانی نشود، همان کدهای خودتان را بگذارید)
        self.setStyleSheet(
            """
            QWidget { background-color: #070B19; color: #F8FAFC; font-family: "Segoe UI", "Arial", sans-serif; }
            QGroupBox { background-color: #0C1327; border: 1px solid #1A2642; border-radius: 6px; margin-top: 20px; }
            QGroupBox::title { subcontrol-origin: margin; subcontrol-position: top left; left: 0px; padding: 0px 8px; background-color: transparent; color: #00E5FF; font-size: 13px; font-weight: 800; }
            QLabel { color: #F8FAFC; font-size: 13px; font-weight: 600; background-color: transparent; }
            QCheckBox { background-color: transparent; color: #F8FAFC; font-size: 12px; font-weight: 600; spacing: 6px; padding: 3px 0px; }
            QCheckBox:hover { color: #00E5FF; }
            QCheckBox:checked { color: #00E5FF; }
            QCheckBox::indicator { width: 16px; height: 16px; border: 1px solid #1A2642; border-radius: 3px; background-color: #03060E; }
            QCheckBox::indicator:hover { border-color: #00E5FF; }
            QCheckBox::indicator:checked { background-color: #00E5FF; border-color: #00E5FF; image: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' viewBox='0 0 24 24' fill='none' stroke='%23070B19' stroke-width='4' stroke-linecap='round' stroke-linejoin='round'><polyline points='20 6 9 17 4 12'></polyline></svg>"); }
            QLineEdit { background-color: #03060E; color: #F8FAFC; border: 1px solid #1A2642; border-radius: 4px; padding: 8px 12px; font-family: "Consolas", monospace; font-size: 13px; font-weight: bold; selection-background-color: #00B4D8; }
            QLineEdit:read-only { background-color: #070B19; color: #94A3B8; border: 1px solid #1A2642; }
            QLineEdit:read-only[hasFile="true"] { border: 1px solid #00E5FF; color: #00E5FF; }
            QPushButton { background-color: #121D38; color: #F8FAFC; border: 1px solid #1A2642; border-radius: 6px; padding: 8px 16px; min-height: 40px; font-size: 13px; font-weight: bold; }
            QPushButton:hover { background-color: #00B4D8; color: #FFFFFF; border-color: #00E5FF; }
            QPushButton:pressed { background-color: #0093B4; }
            QPushButton:disabled { background-color: #0B1220; color: #94A3B8; border-color: #1A2642; }
            QPushButton#applyBtn { border: 1px solid #00B4D8; }
            """
        )
    ...
    def _init_ui(self) -> None:
        ...
        self.btn_reload = QPushButton(" Reload Option Bytes")
        self.btn_reload.setIcon(QIcon(ICON_ARROWS_ROTATE))
        self.btn_reload.setIconSize(QSize(16, 16))
        self.btn_reload.setMinimumHeight(45)
        self.btn_reload.clicked.connect(self._on_reload_clicked)
    ...
    def _on_reload_clicked(self) -> None:
        ...
        self.btn_apply.setEnabled(False)
        self.btn_reload.setText(" Reading Target...")
        self.btn_reload.setIcon(QIcon(ICON_HOURGLASS))
        self.lbl_rdp_status.setStyleSheet(
            "color: #00E5FF; font-size: 12px; font-weight: bold;")

        # 🌟 ارسال تارگت جاری به ورکر
        self._read_worker = OptionBytesReadWorker(
    ...
    def _on_read_finished(self, success: bool, ob_data: Dict[str, Any], error_msg: str) -> None:
        ...
        rdp_raw = ob_data.get("rdp_raw", 0xFF)
        ...
        if rdp_raw == 0xA5:
            ...
            self.lbl_rdp_status.setStyleSheet(
                "color: #10B981; font-size: 12px; font-weight: bold;")
        elif rdp_raw == 0xFF:  # برای بردهای غیر ST
            self.chk_rdp.setChecked(False)
            self.lbl_rdp_status.setText(
                f"Security/RDP logic not mapped for {self.current_mcu_target.upper()}")
            self.lbl_rdp_status.setStyleSheet(
                "color: #F59E0B; font-size: 12px; font-weight: bold;")
        else:
            self.chk_rdp.setChecked(True)
    ...
    def _on_apply_clicked(self) -> None:
        ...
        self.btn_reload.setEnabled(False)
        self.btn_apply.setText(" Programming...")
        self.btn_apply.setIcon(QIcon(ICON_HOURGLASS))

        # 🌟 ارسال تارگت جاری به ورکر رایت
        self._program_worker = OptionBytesProgramWorker(
            rdp_value=rdp_val,
            user_config_byte=user_val,
            target_type=self.current_mcu_target
        )
        self._program_worker.ob_program_finished.connect(
            self._on_program_finished)
