...
Industrial Memory Viewer Widget providing STM32CubeProgrammer-like memory inspection.
Supports customizable base address, size, data width (8/16/32-bit), and live Hex/ASCII dump.
Dynamically updates memory maps based on detected MCU profile.
"""
...
class MemoryViewerWidget(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self.set_mcu_target("auto")  # بارگذاری آدرس‌های پیش‌فرض در شروع
    ...
    def _init_ui(self) -> None:
        ...
        self.table_memory.setStyleSheet(
            ...
            QTableWidget::item:selected { background-color: #0077B6; }
            QHeaderView::section {
                background-color: #0C1327; 
                color: #00E5FF;
                font-weight: bold; 
    ...
    def set_mcu_target(self, target_name: str) -> None:
        """Dynamically populate memory address presets based on the detected or selected MCU."""
    ...
    @Slot(bool, int, list, str)
    def _on_memory_read_finished(self, success: bool, address: int, data: List[int], error_msg: str) -> None:
        self.btn_read.setEnabled(True)
        self.btn_read.setIcon(QIcon(ICON_BOOK_OPEN))
        self.btn_read.setText(" Read Memory")
        ...
        for r in range(rows):
            ...
            for c in range(0, 16, step):
                if c < len(row_bytes):
                    chunk = row_bytes[c: c + step]
                    if step == 4:
                        val = (chunk[3] << 24 | chunk[2] << 16 | chunk[1]
                               << 8 | chunk[0]) if len(chunk) == 4 else 0
                        cell_text = f"{val:08X}"
                    elif step == 2:
                        val = (chunk[1] << 8 | chunk[0]) if len(
                            chunk) == 2 else 0
                        cell_text = f"{val:04X}"
                    else:
                        cell_text = f"{chunk[0]:02X}"
